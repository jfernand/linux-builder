...
/// Fetches uutils + busybox — no build step. Called by `distroless fetch`.
pub fn fetch_new_packages(config_path: &Path, cfg: &Config, force: bool) -> Result<()> {
    let root = load_table(config_path)?;
    for pack in &all_packages(&root)? {
...
/// Fetches and builds uutils + busybox. Called by `distroless build-userland`.
